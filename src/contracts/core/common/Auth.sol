...
library Auth {
    ...
    function setupSecurityCouncil(address _councilAddress) internal {
        if (getRoleMemberCount(Role.SAFETY_COUNCIL) != 0)
            revert err.SAFETY_COUNCIL_ALREADY_EXISTS(_councilAddress, getRoleMember(Role.SAFETY_COUNCIL, 0));
    ...
    function transferSecurityCouncil(address _newCouncil) internal {
        ...
        if (owners < 5) revert err.MULTISIG_NOT_ENOUGH_OWNERS(_newCouncil, owners, 5);
    ...
    function _renounceRole(bytes32 role, address account) internal {
        if (account != msg.sender) revert err.ACCESS_CONTROL_NOT_SELF(account, msg.sender);
    ...
    modifier ensureNotSafetyCouncil(bytes32 role) {
        if (role == Role.SAFETY_COUNCIL) revert err.SAFETY_COUNCIL_NOT_ALLOWED();
        _;
    }
}
