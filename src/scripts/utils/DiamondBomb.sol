...
import {FacetCut, FacetCutAction, Initializer} from "diamond/Types.sol";
import {Diamond} from "diamond/Diamond.sol";
import {FacetData} from "kopio/vm-ffi/ffi-facets.s.sol";
...
contract DiamondBomb {
    ...
    function createFacets(
        ...
        if (initCodeIdxs.length != initDatas.length) {
            revert InitializerMismatch(initCodeIdxs.length, initDatas.length);
        }
        ...
        for (uint256 i; i < initDatas.length; ) {
            initializers[i].initContract = cuts[initCodeIdxs[i]].facetAddress;
