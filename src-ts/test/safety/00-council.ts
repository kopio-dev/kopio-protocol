...
import type { SafetyStateChangeEventObject } from '@/types/typechain/src/contracts/interfaces/KopioCore'
...
describe('Safety Council', () => {
  ...
  describe('#setSafetyStateSet', () => {
    it('correctly sets the safety state', async function () {
      ...
      expect(beforeSafetyState).eq(false)
  ...
  describe('#toggleAssetsPaused', () => {
    ...
    describe('toggle actions only for listed assets', () => {
      ...
      it('can toggle actions for listed kopios', async function () {
        await executeContractCallWithSigners(
          hre.Multisig,
          hre.Diamond,
          'toggleAssetsPaused',
          [[f.Kopio.address], Action.REPAY, false, 0],
          [this.deployer, this.devOne, this.userOne],
        )

        const isPaused = await hre.Diamond.assetActionPaused(Action.REPAY.toString(), f.Kopio.address)
        expect(isPaused).eq(true)
      })

      it('cannot toggle actions for addresses that are not listed collateral assets or kopios', async function () {
    ...
    describe('duration based pausing', () => {
      it('can optionally set a timeout on a given pause command', async function () {
        ...
        expect(depositSafetyState.length).eq(1)
      ...
      it.skip('duration based pause functionality should expire after the duration has passed', async function () {
        ...
        expect(blockNumAfter).eq(blockNumBefore + 1)
        expect(timestampAfter).eq(timestampBefore + sevenDays)
    ...
    describe('event emission', () => {
      it('should emit event ICDP.SafetyStateChange on action changed containing action, asset, and description', async function () {
        ...
        expect(event.action).eq(Action.DEPOSIT)
        expect(event.asset).eq(f.Collateral.address)
        expect(event.description).eq('paused')
      })
    })
  })
})
