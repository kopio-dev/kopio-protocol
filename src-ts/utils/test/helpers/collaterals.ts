import type { ERC20Mock, ERC20Mock__factory, MockOracle } from '@/types/typechain'
...
export const addMockExtAsset = async (args = testCollateralConfig): Promise<TestExtAsset> => {
  ...
  const [fakeFeed, contract]: [FakeContract<MockOracle>, MockContract<ERC20Mock>] = await Promise.all([
    createOracles(hre, args.pyth.id, price),
    (await smock.mock<ERC20Mock__factory>('ERC20Mock')).deploy(name ?? symbol, symbol, decimals ?? 18, 0), //shh
  ])
  ...
  const asset: TestExtAsset = {
    ...
    setOracleOrder: order => hre.Diamond.setOracleTypes(contract.address, order),
